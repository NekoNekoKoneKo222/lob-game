...
const PHASE_LABEL = { morning: "朝", day: "昼", evening: "夕方", night: "夜" };
const WORK_LABEL = { instinct: "本能", insight: "洞察", attachment: "愛着", repression: "抑圧" };
const DEPTS = ["info", "safety", "training", "welfare", "record", "hq"];
const DEPT_LABEL = { info: "情報部", safety: "安全部", training: "訓練部", welfare: "福祉部", record: "記録部", hq: "中央本部" };
export function renderHeader(root, state, actions) {
    clear(root);
    root.append(el("div", { cls: "header-row" }, [
    ...
    const loadInput = root.querySelector('input[type="file"]');
    loadInput.addEventListener("change", () => {
        if (loadInput.files && loadInput.files[0])
            actions.loadSaveFile(loadInput.files[0]);
    });
}
export function renderMap(canvas, state, reg) {
    drawFacilityMap(canvas, state, reg);
}
export function renderStaffPanel(root, state, reg, actions) {
    clear(root);
    ...
    hireBar.append(jobSelect, deptSelect, el("button", {
        cls: "btn small", text: "雇用 (40E)",
        onClick: () => actions.hireStaff(jobSelect.value, deptSelect.value),
    }));
    root.append(hireBar);
...
function renderStaffCard(s, state, reg, actions) {
    const statusLabel = { active: "待機中", resting: "療養中", dead: "死亡", broken: "精神崩壊" };
    const job = reg.jobs[s.jobId];
    const card = el("div", { cls: `staff-card status-${s.status}` });
    card.append(el("div", { cls: "staff-top" }, [
    ...
    card.append(el("div", { cls: "mini-bar-row" }, [
    ...
    card.append(el("div", { cls: "stat-grid", text: "" }, [
...
export function renderAbnormalityPanel(root, state, reg, actions) {
    clear(root);
    root.append(el("div", { cls: "panel-head-row" }, [
        el("h2", { text: "収容中の異常存在" }),
    ...
    for (const inst of state.abnormalities) {
        ...
        if (def.description)
            card.append(el("div", { cls: "ab-desc", text: def.description }));
        if (!inst.inBreach) {
            const staffSel = el("select", {}, [
                el("option", { text: "職員を選択…", attrs: { value: "" } }),
                ...activeStaff.map((s) => el("option", { text: s.name, attrs: { value: s.id } })),
            ]);
            const workSel = el("select", {}, Object.keys(WORK_LABEL).map((w) => el("option", { text: `${WORK_LABEL[w]} (適性 ${def.workAffinity[w]})`, attrs: { value: w } })));
            card.append(el("div", { cls: "ab-actions" }, [
                staffSel, workSel,
                el("button", {
                    cls: "btn tiny",
                    text: "作業実行",
                    onClick: () => {
                        ...
                        actions.doWork(staffSel.value, workSel.value);
                    },
                }),
            ]));
            ...
            const checks = activeStaff.map((s) => {
                const cb = el("input", { attrs: { type: "checkbox", value: s.id } });
                return el("label", { cls: "check-label" }, [cb, el("span", { text: s.name })]);
            });
            const checkBoxRow = el("div", { cls: "check-row" }, checks);
            card.append(checkBoxRow, el("button", {
                cls: "btn tiny danger",
                text: "鎮圧命令",
                onClick: () => {
                    const ids = Array.from(checkBoxRow.querySelectorAll("input:checked")).map((i) => i.value);
                    if (ids.length > 0)
                        actions.suppressOrder(inst.defId, ids);
                },
            }));
        }
        list.append(card);
...
export function renderResearchPanel(root, state, reg, actions) {
    ...
    for (const def of Object.values(reg.research)) {
        ...
        list.append(el("div", { cls: `research-card ${done ? "done" : ""}` }, [
...
export function renderSephirahPanel(root, state, reg, actions) {
    clear(root);
...
export function renderLogPanel(root, state) {
    clear(root);
...
export function renderModPanel(root, actions) {
    clear(root);
    root.append(el("h2", { text: "MOD" }), el("div", { cls: "mod-desc", text: "JSON形式のMODファイルを読み込んで異常存在・E.G.O・イベント等を追加できます。" }), el("label", { cls: "btn small file-btn", text: "MOD JSONを読み込む" }, [
        el("input", { attrs: { type: "file", accept: ".json", style: "display:none", id: "mod-file-input" } }),
    ]));
    const input = root.querySelector("#mod-file-input");
    input.addEventListener("change", () => {
        if (input.files && input.files[0])
            actions.importModFile(input.files[0]);
    });
}
