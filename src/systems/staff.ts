import type { GameState, StaffMember, ModRegistry, StaffRank, Department } from "../types.js";
import { RNG } from "../rng.js";

const RANK_ORDER: StaffRank[] = ["I", "II", "III", "IV", "V", "EX"];
const RANK_UP_EXP: Record<StaffRank, number> = {
...
export function generateRecruitName(rng: RNG): string {
  return `${rng.pick(LAST_NAMES)}${rng.pick(FIRST_NAMES)}`;
}

export function createStaff(
  id: string, jobId: string, department: Department, rng: RNG, reg: ModRegistry
): StaffMember {
  const job = reg.jobs[jobId];
  const base = 8;
  const weights = job?.statWeights ?? {};
  const stat = (key: "courage" | "prudence" | "temperance" | "justice") =>
    base + rng.int(0, 6) + Math.round((weights[key] ?? 0) * 3);
  return {
    id,
...
export function gainExp(staff: StaffMember, amount: number, log: (m: string) => void) {
  if (staff.status === "dead") return;
  staff.exp += amount;
  ...
  if (idx < RANK_ORDER.length - 1) {
    ...
    if (staff.exp >= need) {
      ...
      (["courage", "prudence", "temperance", "justice"] as const).forEach((k) => {
        staff.stats[k] += 2;
      });
...
export function applyDamage(staff: StaffMember, hp: number, sanity: number, log: (m: string) => void) {
...
export function restStaff(state: GameState) {
...
export function hireStaff(state: GameState, jobId: string, department: Department, reg: ModRegistry, rng: RNG, log: (m: string) => void): boolean {
  const cost = 40;
...
export function specialAbilityFor(staff: StaffMember, reg: ModRegistry): string | null {
  if (RANK_ORDER.indexOf(staff.rank) < RANK_ORDER.indexOf("IV")) return null;
