import type { GameState, ModRegistry, StaffMember, AbnormalityInstance } from "../types.js";
import { RNG } from "../rng.js";
import { applyDamage, gainExp, specialAbilityFor } from "./staff.js";

// 収容違反は「発生中の異常存在」ごとに、鎮圧命令を受けた職員が
// tick(500ms相当)ごとに攻防を行うリアルタイム処理。
// UI側はsetIntervalでtick()を呼び出し続ける。

export interface BreachAssignment {
  abnormalityDefId: string;
  staffIds: string[];
}

export function suppressOrder(state: GameState, abnormalityDefId: string, staffIds: string[]): { ok: boolean; reason?: string } {
  const inst = state.abnormalities.find((a) => a.defId === abnormalityDefId);
  if (!inst || !inst.inBreach) return { ok: false, reason: "収容違反中ではない" };
  for (const id of staffIds) {
    const s = state.staff.find((x) => x.id === id);
    if (!s || s.status !== "active") return { ok: false, reason: "出動できない職員が含まれている" };
    s.assignedAbnormalityId = abnormalityDefId;
  }
  return { ok: true };
}

export function breachTick(
  state: GameState, reg: ModRegistry, rng: RNG, log: (m: string) => void
): { resolved: string[] } {
  ...
  for (const inst of state.abnormalities) {
    ...
    if (!def) continue;
    const dispatched = state.staff.filter(
      (s) => s.status === "active" && s.assignedAbnormalityId === inst.defId
    );
    ...
    for (const s of dispatched) {
      ...
      if (ability === "処刑人") dmg *= 1.5; // 収容違反対象へ追加ダメージ
