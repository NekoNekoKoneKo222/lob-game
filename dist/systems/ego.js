...
export function extractEGO(state, reg, abnormalityDefId, kind, rng, log) {
    ...
    if (state.facility.energy < cost)
        return { ok: false, reason: "エネルギーが不足している" };
    ...
    if (kind === "weapon") {
        if (!state.ownedEGO.weapons.includes(targetId))
            state.ownedEGO.weapons.push(targetId);
        log(`E.G.O武器「${reg.weapons[targetId]?.name}」を抽出した。`);
    }
    else {
        if (!state.ownedEGO.armors.includes(targetId))
            state.ownedEGO.armors.push(targetId);
        log(`E.G.O防具「${reg.armors[targetId]?.name}」を抽出した。`);
    }
    return { ok: true };
}
export function equipWeapon(state, staffId, weaponId) {
    const s = state.staff.find((x) => x.id === staffId);
    if (s)
        s.equippedWeaponId = weaponId;
}
export function equipArmor(state, staffId, armorId) {
    const s = state.staff.find((x) => x.id === staffId);
    if (s)
        s.equippedArmorId = armorId;
}
export function equipGift(state, staffId, giftId) {
    const s = state.staff.find((x) => x.id === staffId);
    if (!s)
        return;
...
export function modifyWeapon(reg, weaponId, modId, log) {
