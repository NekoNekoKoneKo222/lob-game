import type { GameState, ModRegistry, Department, WorkType } from "./types.js";
...
import {
  renderHeader, renderMap, renderStaffPanel, renderAbnormalityPanel,
  renderResearchPanel, renderSephirahPanel, renderLogPanel, renderModPanel, UIActions,
} from "./ui/render.js";
...
function renderAll() {
  ...
  renderMap($("facility-canvas") as HTMLCanvasElement, state, reg);
...
const actions: UIActions = {
  advancePhase() {
    const order = ["morning", "day", "evening", "night"] as const;
  ...
  toggleRest(staffId) {
    ...
    else if (s.status === "resting") s.status = "active";
    renderAll();
  },
};

function onEnterPhase(phase: GameState["phase"]) {
  if (phase === "evening") {
...
let breachInterval: number | undefined;
function startBreachLoop() {
  if (breachInterval) window.clearInterval(breachInterval);
