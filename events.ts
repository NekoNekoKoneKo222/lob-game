import type { GameState, ModRegistry, GameEventDef } from "../types.js";
import { RNG } from "../rng.js";

export function checkSephirahUnlock(state: GameState, reg: ModRegistry, log: (m: string) => void) {
...
export function rollEvent(state: GameState, reg: ModRegistry, type: GameEventDef["type"], rng: RNG, log: (m: string) => void): GameEventDef | null {
  const pool = Object.values(reg.events).filter(
    (e) => e.type === type && (e.minDay === undefined || state.day >= e.minDay)
  );
...
function applyEvent(state: GameState, reg: ModRegistry, e: GameEventDef, rng: RNG, log: (m: string) => void) {
  ...
  switch (e.effectKey) {
    ...
    case "info_corruption":
      for (const inst of state.abnormalities) {
        if (rng.chance(30)) inst.discovered = false;
      }
      break;
    case "panic":
      for (const s of state.staff) {
        if (s.status === "active") s.sanity = Math.max(0, s.sanity - rng.int(5, 15));
      }
      break;
    case "red_mist":
      for (const inst of state.abnormalities) {
...
export function checkEndgame(state: GameState, reg: ModRegistry, rng: RNG, log: (m: string) => void) {
...
export function attemptTrueEnding(state: GameState, log: (m: string) => void): boolean {
  const allSephirahCores = Object.keys(state.unlockedSephirah).length >= 0; // 実データはreg側で判定
  const conditionsMet =
    state.endgame.day100Triggered &&
