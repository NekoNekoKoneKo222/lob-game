export function startResearch(state, reg, id, log) {
    const def = reg.research[id];
    if (!def)
        return { ok: false, reason: "存在しない研究" };
    if (state.research.completed.includes(id))
        return { ok: false, reason: "既に完了している" };
...
function applyResearchEffect(state, effectKey) {
    if (!effectKey)
        return;
    switch (effectKey) {
        case "energy_cap_up":
