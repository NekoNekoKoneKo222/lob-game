...
export function assignWork(state, staffId, abnormalityDefId) {
    const staff = state.staff.find((s) => s.id === staffId);
    const ab = state.abnormalities.find((a) => a.defId === abnormalityDefId);
    if (!staff || !ab)
        return { ok: false, reason: "対象が見つからない" };
    if (staff.status !== "active")
        return { ok: false, reason: "この職員は現在作業できない状態" };
    if (ab.inBreach)
        return { ok: false, reason: "収容違反中は作業を指示できない" };
    staff.assignedAbnormalityId = abnormalityDefId;
    return { ok: true };
}
export function resolveWork(state, reg, staff, workType, rng, log) {
    if (!staff.assignedAbnormalityId)
        return null;
...
export function decayCounters(state, reg, log) {
